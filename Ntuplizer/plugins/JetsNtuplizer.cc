...
void JetsNtuplizer::initJetCorrUncertainty( void ){
  ...
  std::cout<< jecAK8UncName_ <<std::endl;
  std::cout<< jecAK4UncName_ <<std::endl;
...
void JetsNtuplizer::fillBranches( edm::Event const & event, const edm::EventSetup& iSetup ){
  ...
  if (doAK4Jets_) {
    ...
    for (const pat::Jet &j : *jets_) {
     ...
     nBranches_->jetAK4_vtxNtracks.push_back(0); //j.j.userFloat("vtxNtracks")); 
     nBranches_->jetAK4_vtx3DVal  .push_back(0); //j.j.userFloat("vtx3DVal")); 
     nBranches_->jetAK4_vtx3DSig  .push_back(0); //j.j.userFloat("vtx3DSig"));
